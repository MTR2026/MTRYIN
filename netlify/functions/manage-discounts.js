...
function getDb() { return neon(process.env.NETLIFY_DATABASE_URL); }

async function initTable(sql) {
  await sql`CREATE TABLE IF NOT EXISTS mtryin_discounts (code VARCHAR(50) PRIMARY KEY, pct INTEGER NOT NULL, created_at TIMESTAMP DEFAULT NOW())`;
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'POST') {
    ...
    if (action === 'add') {
      if (!code || pct === undefined || pct === null || isNaN(pct) || pct < 0 || pct > 100)
