...
async function generatePDF(ticket, label, pvp, name, entryNumber, quantity) {
  return new Promise(async (resolve, reject) => {
    ...
    doc.fontSize(9).fillColor('#b0aca5').font('Helvetica')
      .text('21 JUNIO 2026 · MAD FIGHT STADIUM · SAN SEBASTIÁN DE LOS REYES', pad, sepY + 72, { align: 'center', width: W - pad*2, characterSpacing: 1 });
    ...
    const infoLines = [
      '📍 CÓMO LLEGAR: Sala Groove — Ctra. de Getafe, 32, Pinto, Madrid.',
...
async function registerTicket(code, ticketType, ticketName, buyerName, buyerEmail, entryNumber, totalEntries, pvp, promoCode) {
  try {
    await fetch(`${process.env.URL}/.netlify/functions/validate-ticket`, {
      ...
      body: JSON.stringify({
        ...
        ticketData: { ticket_type: ticketType, ticket_name: ticketName, buyer_name: buyerName, buyer_email: buyerEmail, entry_number: entryNumber, total_entries: totalEntries, pvp, is_free: false, promo_code: promoCode || null },
      }),
    });
    ...
    if (buyer_email) {
      try {
        ...
        for (let i = 0; i < qty; i++) {
          ...
          await registerTicket(code, ticket_type, label, buyer_name, buyer_email, i + 1, qty, finalPvp, pi.metadata.promo_code || null);
        ...
        <body style="background:#0a0a0a;color:#f0ede8;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 20px;">
          ...
          <div style="background:rgba(240,237,232,0.05);border:1px solid rgba(240,237,232,0.1);padding:20px;margin-bottom:20px;">
            <table style="width:100%;font-size:14px;">
              ...
              <tr><td style="color:#b0aca5;padding:5px 0;">Fecha</td><td style="text-align:right;color:#f0ede8;">21 Junio 2026</td></tr>
              <tr><td style="color:#b0aca5;padding:5px 0;">Lugar</td><td style="text-align:right;color:#f0ede8;">Sala Groove, Pinto (Madrid)</td></tr>
          ...
          <div style="background:rgba(240,237,232,0.03);border:1px solid rgba(240,237,232,0.08);padding:20px;margin-bottom:20px;font-size:13px;line-height:1.8;color:#b0aca5;">
            <p>📍 <strong style="color:#f0ede8;">Cómo llegar:</strong> Sala Groove — Av. de los Gavilanes, Pinto, Madrid. Cercanias C-3 hasta Pinto. A 20 min de Madrid en coche.</p>
